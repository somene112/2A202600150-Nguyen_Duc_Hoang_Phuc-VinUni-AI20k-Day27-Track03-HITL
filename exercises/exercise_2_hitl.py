        ...
        analysis = llm.invoke([
            {
                "role": "system",
                "content": (
                    "You are a senior code reviewer. Return a structured PRAnalysis with "
                    "summary, risks, actionable comments, confidence, and confidence reasoning. "
                    "Keep ordinary schema or product changes with unresolved migration/testing "
                    "questions in the human-review band. Do not set confidence below "
                    f"{ESCALATE_THRESHOLD:.2f} solely because a schema/product addition is "
                    "missing a migration or tests; use about 0.60-0.72 for that case. If the "
                    "diff includes a cluster of critical security risks such as MD5 password "
                    "hashing, plaintext token storage, SQL injection, or hard-coded user_id "
                    f"values, treat the review as high risk and set confidence below "
                    f"{ESCALATE_THRESHOLD:.2f} unless the diff itself clearly mitigates those risks."
                ),
            },
            {
                "role": "user",
